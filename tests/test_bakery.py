...
from gino import UninitializedError, create_engine
...
async def test(query, options, sa_engine, api, timeout):
    ...
    for q in qs:
        ...
        with pytest.raises(UninitializedError):
            await q.first()
        if isinstance(query, str) or callable(query) or options:
            assert q.elem is not query
        else:
            assert q.elem is query

        for k, v in options.items():
            assert q.elem.get_execution_options()[k] == v
    ...
    try:
        for q in qs:
            ...
            if api:
                ...
                with pytest.raises(UninitializedError):
                    await eq.first()
            assert eq.elem is not q.elem

            assert eq.elem.get_execution_options()["return_model"]
            assert eq.elem.get_execution_options()["loader"] is User
